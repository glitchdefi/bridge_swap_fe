...
export async function web3Accounts({ accountType, extensions, ss58Format }: Web3AccountsOptions = {}): Promise<
  InjectedAccountWithMeta[]
> {
    ...
    injected
      ...
      .map(async ({ accounts, name: source }): Promise<InjectedAccountWithMeta[]> => {
        try {
          ...
          return mapAccounts(
            source,
            list.filter(({ type }) => (type && accountType ? accountType.includes(type) : true)),
            ss58Format,
          )
        } catch (error) {
...
export async function web3AccountsSubscribe(
  ...
  { extensions, ss58Format }: Web3AccountsOptions = {},
): Promise<Unsubcall> {
  if (!web3EnablePromise) {
    ...
    .map(
      ({ accounts: { subscribe }, name: source }): Unsubcall =>
        subscribe((result): void => {
          ...
          try {
            ;(triggerUpdate() as Promise<void>)?.catch(console.error)
          } catch (error) {
            console.error(error)
          }
        }),
